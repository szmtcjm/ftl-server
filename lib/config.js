...
// 解析项目类别
function parseProject(config,argv){
    var projectName = argv.env;

    fs.readFile('./lib/projectPath.json',function(err,data){
        if(err){
            throw err;
        }

        var pathJson = JSON.parse(data.toString());

        config.public = pathJson[projectName][0];
        config.ftl.base = pathJson[projectName][1];
    });
}
